                ...
                roleRequests.filter(r => r.status === 'PENDING').map((request) => (
                  <tr key={request.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-slate-500 text-xs">{request.user?.email}</div>
                    </td>
                    <td className="px-6 py-4 font-medium text-indigo-600">{request.requestedRole}</td>
                    <td className="px-6 py-4 text-slate-600 italic max-w-xs truncate" title={request.reason}>
