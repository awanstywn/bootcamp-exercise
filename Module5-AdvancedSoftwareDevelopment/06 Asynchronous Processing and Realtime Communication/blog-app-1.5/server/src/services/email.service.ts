...
export class EmailService {
  ...
  static async sendWelcome(email: string) {
    if (!env.SMTP_USER) {
      logger.info(`[Email] Skipping welcome email for ${email} (SMTP not configured)`);
      return;
    }
    ...
    try {
      ...
      logger.info(`[Email] Sent welcome email to ${email}`);
    } catch (error) {
      logger.error(`[Email Error] Failed to send welcome email to ${email}: ${error}`);
    }
  }

  static async sendResetPassword(email: string, token: string) {
    if (!env.SMTP_USER) {
      logger.info(`[Email] Skipping reset password email for ${email} (SMTP not configured)`);
      return;
    }
    ...
    try {
      ...
      logger.info(`[Email] Sent reset password email to ${email}`);
    } catch (error) {
      logger.error(`[Email Error] Failed to send reset password email to ${email}: ${error}`);
    }
  }
  static async sendVerificationEmail(email: string, token: string) {
    if (!env.SMTP_USER) {
      logger.info(`[Email] Skipping verification email for ${email} (SMTP not configured)`);
      return;
    }
    ...
    try {
      ...
      logger.info(`[Email] Sent verification email to ${email}`);
    } catch (error) {
