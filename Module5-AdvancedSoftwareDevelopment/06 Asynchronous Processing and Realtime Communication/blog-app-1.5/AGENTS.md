...
- **Caching & Queues (Server):** Use `ioredis` connected to the Redis container. Implement caching at the **Service layer** (not Route/Controller) to allow programmatic invalidation on mutations (using `scanStream` for robust pattern matching). Use **BullMQ** for any background processing (like scheduling posts or emails).
- **Validation (Server & Client):** Always use `Zod` for runtime validation. Ensure validation schemas correctly match the expected middleware inputs.
