...
export default function AdminUsersPage() {
  ...
  const [pendingRoleChange, setPendingRoleChange] = useState<{ id: string; name: string; role: string; oldRole: string } | null>(null);
  ...
  return (
    <div>
                ...
                roleRequests.filter(r => r.status === 'PENDING').map((request) => (
                  <tr key={String(request.id)} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{String((request.user as Record<string, unknown>)?.name || 'Unknown User')}</div>
                      <div className="text-slate-500 text-xs">{String((request.user as Record<string, unknown>)?.email || '')}</div>
                    </td>
                    <td className="px-6 py-4 font-medium text-indigo-600">{String(request.requestedRole)}</td>
                    <td className="px-6 py-4 text-slate-600 italic max-w-xs truncate" title={String(request.reason || '')}>
      ...
      <ConfirmModal
        ...
        message={
          <p>
            Are you sure you want to change <strong>{pendingRoleChange?.name}</strong>'s role from <span className="font-semibold text-slate-500">{pendingRoleChange?.oldRole}</span> to <span className="font-bold text-indigo-600">{pendingRoleChange?.role}</span>?
