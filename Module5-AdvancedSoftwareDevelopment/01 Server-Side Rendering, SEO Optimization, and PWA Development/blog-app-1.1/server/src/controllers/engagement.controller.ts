...
export class EngagementController {
  ...
  static async toggleLike(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await EngagementService.toggleLike(req.params.postId as string);
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  static async checkLikeStatus(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await EngagementService.checkLikeStatus(req.params.postId as string);
      res.json(result);
    } catch (error) {
      next(error);
    }
