...
- **Validation:** Use manual validation for `req.body`, `req.query`, and `req.params` before the request reaches the controller or directly in the controller logic. Ensure all data matches the shared DTOs.
...
- **Rate Limiting:** Protect high-risk endpoints (e.g., auth endpoints) with `express-rate-limit`.
- **Tokens:** Critical temporary tokens must embed proper validation to prevent replay attacks if a URL is intercepted.
