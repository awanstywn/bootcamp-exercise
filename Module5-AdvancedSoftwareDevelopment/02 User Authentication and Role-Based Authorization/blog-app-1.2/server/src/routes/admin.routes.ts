...
// Update user role
router.patch('/users/:id/role', authenticate, authorize(Role.ADMIN), AdminController.updateUserRole);

// Role Requests
router.get('/role-requests', authenticate, authorize(Role.ADMIN), AdminController.getRoleRequests);
router.patch('/role-requests/:id', authenticate, authorize(Role.ADMIN), AdminController.updateRoleRequestStatus);
