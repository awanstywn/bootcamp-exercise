...
export class AuthService {
  static async register(input: RegisterInput) {
    ...
    EmailService.sendVerificationEmail(input.email, token).catch(e => logger.error('Failed to send verification email:', e));
  ...
  static async verifyEmail(token: string) {
    try {
      ...
      EmailService.sendWelcome(user.email).catch(e => logger.error('Failed to send welcome email:', e));
  ...
  static async forgotPassword(email: string) {
    ...
    EmailService.sendResetPassword(user.email, token).catch(e => logger.error('Failed to send reset password email:', e));
  }
