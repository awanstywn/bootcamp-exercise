...
- **Services (Business Logic & Caching & Queues):** ALL business logic, Prisma database calls, Redis caching logic (`scanStream`), background tasks via BullMQ (like scheduling posts and emails), and data mapping MUST live in the Service layer.
...
- **Rate Limiting:** Protect high-risk endpoints (email verification, password resets, auth endpoints, file uploads) with `express-rate-limit` using `rate-limit-redis` for distributed cluster synchronization.
