    ...
    if (id) {
      ...
      api.get(`/content/posts?status=ALL`)
        .then(res => {
          ...
          const post = res.data.data.find(  (p: any) => p.id === id);
          if (post) {
            setFormData({
              title: post.title,
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              tagsString: post.tags?.map(  (t: any) => t.name).join(', ') || '',
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 space-y-6 shadow-sm">
            ...
            {formData.status === 'SCHEDULED' && (
              <div className="pt-4 border-t border-slate-100">
                <label className="block text-sm font-bold text-slate-700 mb-1">Schedule Date & Time</label>
                <input 
                  type="datetime-local" 
