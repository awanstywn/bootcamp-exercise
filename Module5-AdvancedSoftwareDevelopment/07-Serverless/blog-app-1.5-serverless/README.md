...
- **Caching & Real-Time:** Redis (via `ioredis`) for high-performance data caching, rate-limit synchronization, and BullMQ background task queues (e.g. for scheduled posts and emails).
...
- **Security & Reliability:** Distributed rate limiting (`rate-limit-redis`), HTTP header hardening (`helmet`), robust input validation (`zod`), centralized Winston logging, and graceful shutdowns.
- **Performance:** Response payload `compression`, Redis `scanStream` cache invalidation, and automated Cloudinary CDN optimization.
