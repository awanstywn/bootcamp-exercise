...
// User business logic — handles user CRUD and fetching user-specific posts.
...
// Get all posts authored by a specific user — includes author info and engagement counts
export const getPostsByUserId = async (id: string) => {
  ...
  // Fetch posts with related data using Prisma's `include` and `_count`
  const posts = await prisma.post.findMany({
    ...
    include: {
      ...
      _count: {
        select: {
          comments: true, // Count of comments on each post
          likes: true,    // Count of likes on each post
        },
      },
    },
    orderBy: {
