 ...
 * 
 * UPDATED v2.0: Migrated from Backendless to Express.js + JWT
 */
const useAuthStore = create<AuthStore>()(
  persist(
    (set) => ({
      ...
      /**
       * Authenticates an existing user via Express.js backend.
       * Upon success, stores the user session and JWT token in the global state.
       * CHANGE: Backend now returns { user, token } instead of BackendlessUser with nested user-token.
       */
      ...
      /**
       * Registers a new user via Express.js backend.
       */
