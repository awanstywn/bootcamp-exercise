...
- **SSR Compatibility:** Be cautious when accessing browser-only APIs (`window`, `document`, `localStorage`). Ensure these are accessed only in `useEffect` hooks or after verifying `typeof window !== 'undefined'` since this app utilizes Server-Side Rendering.
