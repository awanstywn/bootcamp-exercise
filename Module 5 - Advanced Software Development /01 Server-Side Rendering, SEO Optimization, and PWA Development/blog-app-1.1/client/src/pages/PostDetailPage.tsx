  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      ...
      try {
        ...
        setRelated(relRes.data.data.filter((p: RelatedPost) => p.id !== data.id).slice(0, 3));
        setTags(tagsRes.data);
        setCategories(catRes.data);

        try {
          const likeRes = await api.get(`/content/posts/${data.id}/likes/status`, {
            signal: controller.signal,
          });
          setHasLiked(likeRes.data.liked);
        } catch (_err) {
          // Ignore error if auth is missing or endpoint fails
        }
      } catch (error: unknown) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-500 mb-8 pb-4 border-b border-slate-200">
        ...
        <span>&rsaquo;</span>
        <span className="text-slate-900 font-medium truncate max-w-[200px] md:max-w-xs">
          {post.title}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 lg:gap-16">
            ...
            {post.content ? (
              post.content.split('\n\n').map((paragraph: string, i: number) => {
                if (paragraph.startsWith('# '))
                  return (
                    <h1 key={i} className="text-3xl mt-8 mb-4 font-bold">
                      {paragraph.replace('# ', '')}
                    </h1>
                  );
                if (paragraph.startsWith('## '))
                  return (
                    <h2 key={i} className="text-2xl mt-8 mb-4 font-bold">
                      {paragraph.replace('## ', '')}
                    </h2>
                  );
                if (paragraph.startsWith('### '))
                  return (
                    <h3 key={i} className="text-xl mt-6 mb-3 font-bold">
                      {paragraph.replace('### ', '')}
                    </h3>
                  );
                if (paragraph.startsWith('> '))
        ...
        <aside className="space-y-10 pl-0 lg:pl-4">
          {/* Categories */}
          <div>
            <h3 className="font-bold text-slate-900 mb-4 font-serif border-b border-slate-200 pb-2">
              Categories
            </h3>
              ...
              {related.map((rel) => (
                ...
                >
                  ...
                  <div className="flex flex-col justify-center">
                    <h4 className="font-bold font-serif text-xs line-clamp-2 group-hover:text-slate-600 transition-colors">
                      {rel.title}
                    </h4>
                    <span className="text-[10px] text-slate-500 mt-1">
                      {rel.createdAt ? format(new Date(rel.createdAt), 'MMM dd, yyyy') : ''}
                    </span>
                  </div>
                </Link>
              ))}
