    ...
    const fetchSidebarData = async () => {
      try {
        const [popularRes] = await Promise.all([
          api.get('/content/posts?limit=4&status=PUBLISHED&sort=popular', {
    ...
    const fetchPosts = async () => {
      ...
      try {
        const res = await api.get(
          `/content/posts?limit=6&status=PUBLISHED&page=${page}&sort=${sort}`,
          { signal: controller.signal },
        );
      ...
      <div className="flex flex-col md:flex-row items-center gap-12 mb-16 pb-16 border-b border-slate-200">
        <div className="flex-1 space-y-6">
          ...
          <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
            Read articles on technology, design, development and more.
          </p>
          <div>
            <button
              onClick={() => {
          ...
          {isPostsLoading ? (
            <div className="space-y-8">
              {[1, 2, 3].map((n) => (
                <div
                  key={n}
                  className="animate-pulse bg-slate-50 h-48 rounded-lg border border-slate-100"
                />
              ))}
            </div>
          ) : posts.length > 0 ? (
              ...
              {popularPosts.map((post, index) => (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    <h4 className="font-bold font-serif text-sm leading-snug line-clamp-2 group-hover:text-slate-900 transition-colors">
                      {post.title}
                    </h4>
