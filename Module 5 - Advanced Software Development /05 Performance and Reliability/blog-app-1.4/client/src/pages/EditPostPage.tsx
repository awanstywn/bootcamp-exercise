...
export default function EditPostPage() {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      api.get(`/content/posts?status=ALL`)
        .then(res => {
          const post = res.data.data.find(/* eslint-disable-next-line @typescript-eslint/no-explicit-any */ (p: any) => p.id === id);
          if (post) {
            setFormData({
              title: post.title,
              ...
              tagsString: post.tags?.map(/* eslint-disable-next-line @typescript-eslint/no-explicit-any */ (t: any) => t.name).join(', ') || ''
            });
          }
        })
        .catch(console.error);
