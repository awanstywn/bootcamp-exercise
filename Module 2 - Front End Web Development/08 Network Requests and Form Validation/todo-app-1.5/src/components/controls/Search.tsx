 ...
 * Search Component
 * 
 * Objective: Provides a unified control bar containing the real-time search input, 
 * as well as toggle buttons for the Sort and Filter dropdown menus.
 * 
 * Logic & Behavior:
 * - Binds the search text directly to the global `useTodoStore` for instant list filtering.
 * - Manages the visibility of the Sort and Filter dropdowns using local component state.
 * - Implements a "click-outside" listener via `useEffect` and `useRef` to automatically 
 *   close active dropdowns when the user taps elsewhere on the page.
 */
...
export function Search() {
  ...
  const searchInput = useTodoStore((s) => s.searchInput);
  const setSearchInput = useTodoStore((s) => s.setSearchInput);
