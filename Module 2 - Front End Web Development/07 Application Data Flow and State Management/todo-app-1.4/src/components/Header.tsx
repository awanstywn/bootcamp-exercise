/**
 * Header — App title ("TODO"), theme toggle, and burger menu overlay.
 *
 * How this component works:
 * ─────────────────────────
 * 1. The header shows two things on the right side:
 *    - A theme toggle button (sun/moon icon)
 *    - A burger menu button (☰ three horizontal lines)
 *
 * 2. When the user clicks the burger button:
 *    - A dark semi-transparent BACKDROP covers the entire screen
 *    - A PANEL slides in from the right edge of the screen
 *    - The panel displays the user's profile info and a "Sign out" button
 *
 * 3. The overlay can be closed in 3 ways:
 *    - Clicking the ✕ close button inside the panel
 *    - Clicking the dark backdrop area (outside the panel)
 *    - Pressing the Escape key on the keyboard
 *
 * State management:
 * - `menuOpen` (useState) — boolean that controls whether the overlay is visible
 * - When `menuOpen` is true  → backdrop fades in, panel slides in from right
 * - When `menuOpen` is false → backdrop fades out, panel slides back off-screen
 *
 * Styling:
 * - All overlay styles are defined in index.css using custom CSS classes
 * - The `open` class is toggled on/off to trigger CSS transitions
 */
...
export function Header({ isDarkMode }: HeaderProps) {
  ...
  // `user` is either a RegularUser, GuestUser, or null (not logged in)
      ...
      */}
      ...
      >
        ...
        {/* ── User Avatar Circle ──
            ...
            For guest users, it shows "G".
            Uses optional chaining (?.) in case user is null.
        */}
        <div className="menu-overlay-avatar">
          {user?.type === "user" ? user.name.charAt(0).toUpperCase() : "G"}
        </div>

        {/* ── User Info ──
            Shows the user's name and email (if they're a regular user).
            For guest users, just shows "Guest" with no email.
            The `user &&` check ensures this block only renders when someone is logged in.
        */}
        {user && (
          <div className="menu-overlay-user-info">
            <span className="menu-overlay-name">
              {user.type === "user" ? user.name : "Guest"}
            </span>
            {/* Only show email for regular users (guests don't have one) */}
            {user.type === "user" && (
