...
**New in v1.4:** The app now includes a complete **authentication system** with a Sign In page, guest login, and protected routing via React Router. The header features a **burger menu overlay** that slides in from the right to display the user's profile and a sign-out button. The Sign In page matches the Todo page's visual identity (banner image + gradient overlay).
...
| Routing          | **React Router** (`react-router-dom`)                                     | Client-side routing for Sign In ↔ Todo page navigation   |
...
| F8  | Authentication     | Sign In page with name/email login, guest login, protected routing               |
...
| ID     | User Story                                                                                                                   | Priority  |
| ------ | ---------------------------------------------------------------------------------------------------------------------------- | --------- |
| US-18  | As a user, I can **sign in with my name and email** so the app knows who I am.                                               | Must Have |
| US-18b | As a user, I can **continue as a guest** without entering credentials.                                                       | Must Have |
| US-19  | As a user, if I am **not logged in**, I am **redirected to the Sign In page** automatically.                                 | Must Have |
| US-19b | As a user, if I am **already logged in** and visit `/signin`, I am **redirected to the home page** automatically.            | Must Have |
...
| US-23b | As a guest user, the avatar shows **"G"** and the name shows **"Guest"** (no email).                                                  | Must Have |
...
│   │
│   ├── page/
│   │   └── SignIn.tsx                 # Sign In page (form + guest login + theme toggle)
│   │
│   ├── store/
│   │   ├── useTodoStore.ts            # Zustand store — todo state + actions
│   │   └── useAuthStore.ts            # Zustand store — auth state + actions (login/logout)
│   │
│   ├── hooks/
...
│   │
│   ├── types/
│   │   └── types.ts                   # All shared TypeScript types (incl. auth types)
│   │
│   ├── utils/
...
│   │
│   ├── App.tsx                        # Root component (routing, layout, banner, content)
...
- User avatar circle — shows first letter of user's name (or "G" for guest)
- User name and email (or just "Guest")
- Horizontal divider
- "Sign out" button — clears auth state and redirects to `/signin`
...
Props:
  isDarkMode: boolean
Store state used: user (from useAuthStore)
Store actions used: toggleTheme (from useTodoStore), logout (from useAuthStore)
...
Full-page sign-in form styled to match the Todo page layout. Uses the same banner area (background image + gradient overlay) and AppShell wrapper as TodoPage.

**Layout structure (mirrors TodoPage):**
1. AppShell wrapper → applies `.dark` class
2. Banner area at top → background image + purple gradient overlay
3. Content floated above (z-10) → "SIGN IN" title + theme toggle + form card

**Form contents:**
- Name input (controlled, required)
- Email input (controlled, required)
- "Sign In" button → calls `login(name, email)` then navigates to `/`
- Divider ("or")
- "Continue as Guest" button → calls `loginAsGuest()` then navigates to `/`

**Theme toggle:** Sun/moon icon button in the header area, calls `toggleTheme()` so the user can switch themes before logging in.

```
Props: none
Store state used: isDarkMode (from useTodoStore)
Store actions used: login, loginAsGuest (from useAuthStore), toggleTheme (from useTodoStore)
Local state: name, email (form input values)
...
interface RegularUser {
  type: "user";    // Literal type tag — discriminant
  name: string;
  email: string;
}

/** A guest user — no credentials */
interface GuestUser {
  type: "guest";   // Literal type tag — discriminant
}

/** Discriminated union: User is either RegularUser or GuestUser.
...
type AuthStore = {
  ...
  // ── Actions ────────────────────────
  login: (name: string, email: string) => void;  // save RegularUser
  loginAsGuest: () => void;                       // save GuestUser
  logout: () => void;                             // clear user to null
};
```

**Why a discriminated union?** By checking `user.type`, TypeScript can narrow the type and give you autocompletion. For example:

```typescript
if (user.type === "user") {
  // TypeScript knows: user has .name and .email
  console.log(user.name, user.email);
} else {
  // TypeScript knows: user is GuestUser (no .name or .email)
}
```
...
1. **`useTodoStore`** — manages todos, theme, search, sort, and filter
2. **`useAuthStore`** — manages authentication state (user data, login/logout)
...
### Auth Store (`useAuthStore`) — new in v1.4
...
const useAuthStore = create<AuthStore>()(
  persist(
    (set) => ({
      ...
      isLoggedIn: false,   // Derived from user !== null
...
**Key differences from `useTodoStore`:**
- Uses `persist` **without `partialize`** — all fields are persisted (user + isLoggedIn)
- Stored under a **separate localStorage key** (`"auth-storage"`) so auth and todo data are independent
- No `get()` needed — all actions only use `set()` since they don't read existing state
...
### 8.8 Authentication Flow (new in v1.4)
...
```
User opens app
  → App reads isLoggedIn from useAuthStore
  → If false → redirect to /signin
  → User fills in name + email → clicks "Sign In"
  → login(name, email) stores { type: "user", name, email } in auth store
  → navigate("/", { replace: true }) → shows TodoPage
```

**Guest login flow:**
```
User clicks "Continue as Guest"
  → loginAsGuest() stores { type: "guest" } in auth store
  → navigate("/", { replace: true }) → shows TodoPage
```
...
User clicks burger menu → clicks "Sign out"
  ...
  → logout() sets user = null, isLoggedIn = false
  → navigate("/signin", { replace: true }) → shows SignIn page
```

**Persistence:** Auth state is saved to `localStorage` under key `"auth-storage"`. On page refresh, Zustand's `persist` middleware rehydrates the store, so the user stays logged in.

---

### 8.9 Burger Menu Overlay Pattern (new in v1.4)
...
### 8.10 Overlay CSS Architecture (index.css)
